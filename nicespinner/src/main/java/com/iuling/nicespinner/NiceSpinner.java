...
public class NiceSpinner extends AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        //初始化padding
        int padding = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_padding, -1);
        if (padding != -1)
            setPadding(padding, padding, padding, padding);
        else {
            int paddingLeft = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_paddingLeft, -1);
            if (paddingLeft == -1)
                paddingLeft = defaultPadding;

            int paddingRight = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_paddingRight, -1);
            if (paddingRight == -1)
                paddingRight = defaultPadding;

            int paddingTop = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_paddingTop, -1);
            if (paddingTop == -1)
                paddingTop = defaultPadding;

            int paddingBottom = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_paddingBottom, -1);
            if (paddingLeft == -1)
                paddingLeft = defaultPadding;
        ...
        //初始化边框画笔
        borderColor = typedArray.getColor(R.styleable.NiceSpinner_borderColor, -1);
        borderWidth = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_borderWidth, 1);
        if (borderColor != -1) {
        ...
        dividerColor = typedArray.getColor(R.styleable.NiceSpinner_dividerColor, getDefaultTextColor(context));
        dividerHeight = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_dividerHeight, getDefaultTextColor(context));

        itemGravity = typedArray.getInt(R.styleable.NiceSpinner_itemGravity, Gravity.CENTER);
        ...
        if (dividerColor != 0){
            ...
            if (dividerHeight == 0)
                dividerHeight = dip2px(context,1.0f);
    ...
    public <T> void attachDataSource(List<T> list) {
        adapter = new NiceSpinnerAdapter<>(getContext(), list, textColor, backgroundSelector,
                itemGravity,spinnerTextFormatter);
